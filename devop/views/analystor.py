...
def getweek(date):
    dic = {0:u'一', 1:u'二', 2:u'三', 3:u'四', 4:u'五', 5:u'六', 6:u'日'}
...
def twoandmore(mydate, firstime, lastime):
    ...
    if flag8 < firstime < flag1:
        ...
        if lastime < flag4:
            ...
            plus = None
            leave = flag4 - lastime
            return [firstime, lastime, late, plus, leave.seconds/3600, '提前下班了，要请假的节奏']
    ...
    if firstime > flag3:
        ...
        late = smtime.seconds/3600
        print('迟到%s,继续判断下班时间' % smtime)
        if lastime < flag6:
            print('早退了吗？计算早退时间')
            plus = None
            leave = flag6 - lastime
            return [firstime, lastime, late, plus, leave.seconds/3600, '迟到+早退']
        if flag6 < lastime < flag7:
            print('下班时间ok')
            plus = None
            leave = None
...
def gethours(username, mydate, lst, args):
    ...
    if is_workday(mydate):
        ...
        if len(args) == 0:
            print(mydate + ':' + '全天请假')
            return {'username': username, 'date':mydate, 'myweek':week, 'on': None, 'off': None, 'late': None, 'plus': None, 'leave': 8.0, 'content': '全天请假'}
        if len(args) == 1:
            print('发现打卡一次')
            if args[0] < flag8:
                print('此次为凌晨打卡，根据规定，凌晨打卡的加班时间算在了前一天的加班时间里，所以今天判断应该是休息了一天')
                return {'username': username, 'date':mydate, 'myweek':week, 'on': None, 'off': None, 'late': None, 'plus': None, 'leave': 8.0, 'content': '凌晨有1次打卡，判断昨天加班今天没上班'}
            else:
                # 判断第二天凌晨是不是有打卡
                if len(lst) != 0:
                    if heapq.nsmallest(1, lst)[0] < flag0:
                        ...
                        if args[0] < flag1:
                            ...
                            if plus < 1.5:
                                ...
                                pass
                            print('加班%s' % overtime)
                            return {'username': username, 'date':mydate, 'myweek':week, 'on': args[0], 'off': heapq.nsmallest(1, lst), 'late': None, 'plus': plus, 'leave': None, 'content': '次日凌晨加班时间累计到今天'}
                    ...
                    if args[0] < flag3:
                        ...
                        if args[0] > flag9:
                            ...
                            if smtime > timedelta(hours=3, minutes=0, seconds=0):
                                ...
                                return {'username': username, 'date':mydate, 'myweek':week, 'on': None, 'off': args[0], 'late': None, 'plus': None, 'leave': None, 'content': '判断为上班忘记打卡'}
                            else:
                                print('此次打卡为上午，应该是迟到了，计算迟到时间')
                                chidao = args[0] - flag3
                                print('迟到%s' % chidao)
                                return {'username': username, 'date':mydate, 'myweek':week, 'on': args[0], 'off': None, 'late': chidao.seconds/3600, 'plus': None, 'leave': None, 'content': '判断为下班忘记打卡并且上班迟到了'}
        if len(args) == 2:
            print('发现打卡2次')
                ...
                if heapq.nsmallest(1, lst)[0] < flag0:
                    ...
                    if args[0] < flag1:
                        ...
                        if plus < 1.5:
                            ...
                            pass
                        print('加班%s' % overtime)
                        return {'username': username, 'date':mydate, 'myweek':week, 'on': args[0], 'off': heapq.nsmallest(1, lst)[0], 'late': None, 'plus': plus, 'leave': None, 'content': '次日凌晨加班时间累计到今天'}
                    if flag1 < args[0] < flag2:
                        print('9点到9点半之间来的哦')
                        overtime = heapq.nsmallest(1, lst)[0] - flag6
                        plus = overtime.seconds / 3600
                    ...
                    if args[0] > flag9:
                        ...
                        if plus < 1.5:
                            ...
                            pass
                        print('加班%s' % overtime)
                        return {'username': username, 'date':mydate, 'myweek':week, 'on': args[0], 'off': heapq.nsmallest(1, lst)[0], 'late': smtime.seconds/3600, 'plus': plus, 'leave': None, 'content': '次日凌晨加班时间累计到今天'}
                else:
                    print('次日凌晨没有打卡，判断为忘记打卡一次，有问题人工审核')
                    if args[0] < flag3:
                        print('次日凌晨没有打卡，也没发现迟到，判断为忘记打卡一次，有问题人工审核')
                        ...
                        if args[0] > flag3:
                            ...
                            if smtime > timedelta(hours=3, minutes=0, seconds=0):
                                ...
                                if args[0] > flag6:
                                    print('下午没有早退')
                                    return {'username': username, 'date':mydate, 'myweek':week, 'on': None, 'off': args[0], 'late': None, 'plus': None, 'leave': None, 'content': '判断为上班忘记打卡，人工审核'}
                                else:
                                    print('可能早退了,人工审核')
                                    return {'username': username, 'date':mydate, 'myweek':week, 'on': None, 'off': args[0], 'late': None, 'plus': None, 'leave': None, 'content': '判断为上班忘记打卡，人工审核'}
                            else:
                                ...
                                chidao = args[0] - flag3
                                print('迟到%s' % chidao)
                                return {'username': username, 'date':mydate, 'myweek':week, 'on': args[0], 'off': None, 'late': chidao.seconds/3600, 'plus': None, 'leave': None, 'content': '判断为下班忘记打卡，人工审核'}
            else:
                if firstime < flag8:
                    ...
                    if heapq.nsmallest(1, lst)[0] < flag0:
                        ...
                        if args[0] < flag3:
                            ...
                            if args[0] > flag9:
                                ...
                                if args[0] < flag3:
                                    print('看起来没有迟到')
                                    return {'username': username, 'date':mydate, 'myweek':week, 'on': args[0], 'off': None, 'late': None, 'plus': None, 'leave': None, 'content': '判断为下班忘记打卡，人工审核'}
                                else:
                                    print('似乎迟到了，计算迟到时间')
                                    smtime = args[0] - flag3
                                    print('迟到%s' % smtime)
                                    return {'username': username, 'date':mydate, 'myweek':week, 'on': args[0], 'off': None, 'late': smtime.seconds/3600, 'plus': None, 'leave': None, 'content': '判断为下班忘记打卡，人工审核'}
                else:
                    firstime = heapq.nsmallest(1, args)[0]
                    lastime = heapq.nlargest(1, args)[0]
                    ll = twoandmore(mydate, firstime, lastime)
                    return {'username': username, 'date':mydate, 'myweek':week, 'on': ll[0], 'off': ll[1], 'late': ll[2], 'plus': ll[3], 'leave': ll[4], 'content': ll[5]}
        if len(args) >= 3:
        ...
        if len(args) == 1:
            print('加班只打一次卡，人工审核')
            return {'username': username, 'date':mydate, 'myweek':week, 'on': args[0], 'off': None, 'late': None, 'plus': None, 'leave': None, 'content': '加班只打一次卡,人工审核'}
        if len(args) == 2:
            ...
            if plus < 1.5:
                ...
                pass
            print('加班%s' % overtime)
            return {'username': username, 'date':mydate, 'myweek':week, 'on': args[0], 'off': args[1], 'late': None, 'plus': plus, 'leave': None, 'content': '假日加班'}
        if len(args) >= 3:
            firstime = heapq.nsmallest(1, args)[0]
            lastime = heapq.nlargest(1, args)[0]
            ...
            if plus < 1.5:
                ...
                pass
            return {'username': username, 'date':mydate, 'myweek':week, 'on': firstime, 'off': lastime, 'late': None, 'plus': plus, 'leave': None, 'content': '假日加班'}
