...
from opman.models import Server_Assets
from devop.data.DsRedisOps import DsRedis
...
def apps_add(request):
    if request.method == "GET":
        ...
        return render(request, 'apps/apps_playbook_config.html',
                      {"user": request.user, "userList": userList, "serverList": serverList, "groupList": groupList})
...
def apps_playbook_run(request, pid):
    try:
        ...
        return render(request, 'apps/apps_playbook.html', {"user": request.user, "errorInfo": "剧本不存在，可能已经被删除."})
    if request.method == "GET":
        return render(request, 'apps/apps_playbook.html',
                      {"user": request.user, "playbook": playbook, "serverList": serverList, "numberList": numberList})
    elif request.method == "POST":
...
def apps_playbook_modf(request, pid):
    try:
        ...
        return render(request, 'apps/apps_playbook_modf.html', {"user": request.user, "errorInfo": "剧本不存在，可能已经被删除."})
    if request.method == "GET":
        ...
        return render(request, 'apps/apps_playbook_modf.html',
                      {"user": request.user, "userList": userList, "playbook": playbook, "serverList": serverList,
                       "groupList": groupList})
    elif request.method == "POST":
        try:
        ...
        if request.POST.getlist('playbook_server', None):
            ...
            for sid in request.POST.getlist('playbook_server'):
                try:
                    ...
                    return render('apps/apps_playbook_modf.html', {"user": request.user,
